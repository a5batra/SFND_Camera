...
void magnitudeSobel()
{
    ...
    cv::Mat imgSmooth = imgGray.clone();
    int filterSize = 5;
    int sigmaX = 2.0;
    cv::GaussianBlur(imgGray, imgSmooth, cv::Size(filterSize, filterSize), sigmaX);
    ...
    cv::filter2D(imgSmooth, result_x, -1, kernel_x, cv::Point(-1, -1), 0, cv::BORDER_DEFAULT);
    cv::filter2D(imgSmooth, result_y, -1, kernel_y, cv::Point(-1, -1), 0, cv::BORDER_DEFAULT);
